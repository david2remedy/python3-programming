...
def conjugation(*fuzzies):
    return FuzzyBool(min([float(fuzzy) for fuzzy in fuzzies]))


def disjunction(*fuzzies):
    return FuzzyBool(max([float(fuzzy) for fuzzy in fuzzies]))
...
class FuzzyBool(float):
    ...
    def __repr__(self):
        return "{0}({1})".format(self.__class__._name__, super().__repr__())
